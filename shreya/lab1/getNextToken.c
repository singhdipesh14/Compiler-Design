...
#include <string.h>
#include <ctype.h>
#include "remove_preprocessors.h"
#define MAX_SIZE 20
...
int isDatatype(char buffer[]) { // function to check for data_Type
  for (int i = 0; i < 4; i++) {
    if (strcmp(buffer, data_types[i]) == 0) return 1;
  }
  return 0;
}
...
void display_st() // display the symbol table
{
  ...
  for (int i = 0; i < MAX_SIZE; i++) {
    if (st[i] != NULL) {
      Symbol cur = st[i];
      while (cur != NULL) {
        printf("%10s    |%10s    |%10d    \n", cur -> cur, cur -> data_type, cur -> size);
        cur = cur -> next;
      }
    }
  }
...
int search_symbol(char identifier[], char data_type[]) // to search in symbol_table
{
  ...
  int i = 0;
  while (cur != NULL) {
    if (strcmp(identifier, cur -> cur) == 0) 
      return i;
    cur = cur -> next;
    i++;
  }
...
void insert_symbol(char identifier[], char data_type[]) { // insert in symbol table
  ...
  n->data_type = typee;
    if (strcmp(data_type, "int") == 0) n -> size = 4;
    else if (strcmp(data_type, "double") == 0) n -> size = 8;
    else if (strcmp(data_type, "char") == 0) n -> size = 1;
  else if (strcmp(data_type, "function") == 0) n->size = 0;
    else n -> size = 4;
    int index = hash(strlen(identifier));
    printf("%10s    |%10s    |%10d    \n",n->cur, n-> data_type, n -> size);
    if (st[index] == NULL) {
      st[index] = n;
      return;
    }
...
void insert(char buffer[], int row, int col, enum TYPE type) { // insert in hash table
...
int main() {
  ...
  FILE * finp = fopen("prep_output.c", "r");
  if (finp == NULL) {
    printf("Cannot Find file ");
    return 0;
  }
  char buffer[100], data_type_buffer[100], c = 0;
  int i = 0, col_pos = 1, col,row=1;
  ...
  while (c != EOF) {
    if (isalpha(c) != 0 || c == '_') {
      ...
      } else if (isBracket(c) == 1) { // parentheses and special symbols
        char temp_string[2] = {
          c,
          '\0'
        };
        insert(temp_string, row, col - 1, BRACKET);
      } else if (isSpecial(c) == 1) { // parentheses and special symbols
        char temp_string[2] = {
          c,
          '\0'
        };
        insert(temp_string, row, col - 1, SPECIAL_SYMBOL);
      } else if (isOperator(c) == 1) { // operators
        char temp = c;
        c = fgetc(finp);
        col_pos++;
        if (c == '=' || (temp == '+' && c == '+') || (temp == '-' && c == '-')) {
