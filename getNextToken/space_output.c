...
int main()
{
 space();  int row = process();  for (int i = 0; i < MAX_SIZE; i++)
 hashTable[i] = NULL;
 FILE *finp = fopen("space_output.c", "r");
 if (finp == NULL)
 {
 ...
 if (compare(buffer) == 1)
 {
 insert(buffer, row, col - 1, 1);  }
 else
 {
 insert("id", row, col - 1, 0);  }
 i = 0;
 if (c == '\n')
 row++, col_global = 1;
 buffer[0] = '\0';
 ...
 buffer[i] = '\0';
 insert(buffer, row, col - 1, 0);  buffer[0] = '\0';
 i = 0;
 c = fgetc(finp);
 col_global++;
 }
